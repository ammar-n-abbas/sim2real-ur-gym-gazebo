...
class UR5JointSpaceEnv(ur5_env.UR5Env):
    def __init__(self):
    ...
    def _set_init_pose(self):
        ...
        cpose = transformations.pose_from_angular_velocity(cpose, deltax, dt=self.reset_time, ee_rotation=True)
        ...
        if self.random_initial_pose:
            ...
            self.ur5_arm.set_joint_positions(position=qc,
                                              wait=True,
                                              t=self.reset_time)
        self.max_distance = spalg.translation_rotation_error(self.ur5_arm.end_effector(), self.target_pos) * 1000.
        self.max_dist = None
    ...
    def _compute_reward(self, observations, done):
        ...
        # return -(np.linalg.norm(self.target_pos[:3] - self.ur5_arm.end_effector()[:3])) * 100
    ...
    def _is_done(self, observations):
        ...
        true_error = spalg.translation_rotation_error(self.true_target_pose, self.ur5_arm.end_effector())
        true_error[:3] *= 1000.0
    ...
    def _set_action(self, action):
        ...
        q_cmd = self.ur5_arm.joint_angles() + np.deg2rad(actions) / 10.

        self.ur5_arm.set_joint_positions_flex(q_cmd, t=self.agent_control_dt)
        self.rate.sleep()
