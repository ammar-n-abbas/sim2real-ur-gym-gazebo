...
class RescaleAction(gym.ActionWrapper):
    def __init__(self, env, a, b):
        assert isinstance(env.action_space, spaces.box.Box), (
...
class ReplayBufferSRLA(object):
    def __init__(self, state_dim, action_dim, max_size=int(1e6)):
        self.max_size = max_size
        self.ptr = 0
        self.size = 0

        self.transition_names = ('state', 'option', 'action', 'next_state', 'next_option', 'reward', 'not_done')
        sizes = (state_dim, 1, action_dim, state_dim, 1, 1, 1)
        for name, size in zip(self.transition_names, sizes):
            setattr(self, name, np.empty((max_size, size)))

    def add(self, state, option, action, next_state, next_option, reward, done):
        values = (state, option, action, next_state, next_option, reward, 1. - done)
...
class Actor(Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.net = Mlp(state_dim, [512, 512, 512], 2 * action_dim)
